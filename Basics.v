...
Theorem andb_true_elin1 : forall b c : bool,
...
(* Solution.
   In Coq's world, as we've seen, the recursive functions can be defined using Fixpoint.
   However, for some reason that we'll discuss later, Coq only allow terminating functions,
   that is functions should not perform recursion infinitely. For instance: this function f
...
Fixpoint f (n: nat): nat :=
  ...
  will not accepted by Coq as it will not terminate for any coming arugments. You may notice
  that the function even does NOT compile at all.

  Then the one natural problem is: how does Coq judge statically whether or not one function
  will terminate when run. Of course, this problem is not computable. So Coq adopted a
  conservative solution: there is at least one argument decreacing for recursive call. However,
  this strategy is too coarse in some circumstance, for example, this function does terminate
  for any input nats, but Coq complains...
...
Lemma orb_commutative: 
  forall b c : bool, orb b c = orb c b.
Proof.
  intros [] [].
  - reflexivity.
  - reflexivity.
...
Proof.
  intros [] [].
  - reflexivity.
  - rewrite -> andb_commutative. rewrite -> andb_l_false.
    rewrite -> orb_l_true.
    intros H.
    rewrite -> H.
    reflexivity.
  - rewrite -> andb_l_false.
    rewrite -> orb_commutative. rewrite -> orb_l_true.
    intros H.
    rewrite -> H.
    reflexivity.
  - reflexivity.
...
Example test_bin_incr1 : bin_to_nat (incr Z) = S (bin_to_nat Z).
simpl. reflexivity. Qed.

Example test_bin_incr2 : bin_to_nat (incr (A Z)) = S (bin_to_nat (A Z)).
simpl. reflexivity. Qed.

Example test_bin_incr3 : bin_to_nat (incr (B Z)) = S (bin_to_nat (B Z)).
simpl. reflexivity. Qed.

Example test_bin_incr4 : bin_to_nat (incr (B (B Z))) = S (bin_to_nat (B (B Z))).
simpl. reflexivity. Qed.

Example test_bin_incr5 : bin_to_nat (incr (A (A Z))) = S (bin_to_nat (A (A Z))).
simpl. reflexivity. Qed.
(* Do not modify the following line: *)
Definition manual_grade_for_binary : option (nat*string) := None.
(** [] *)

(* Sat Aug 9 19:13 EST 2019 *)
