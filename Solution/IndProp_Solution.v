...
(** This definition is different in several crucial respects from previous
    ...
    of [ev_SS]. The overall type of each constructor must be specified
    after a colon; each constructor type must have the form [even n]
   ...
   This definition names the [X] parameter  _globally_, to the _left_ 
   of the colon, forcing the result of [nil] and [cons] to be the 
   same ([list X]).  Had we tried to bring  [nat] to the left in 
...
(** The [inversion] tactic does quite a bit of work. When
    ...
    use it to reprove some theorems from [Tactics.v].  Here we are being
    a bit lazy by omitting the [as] clause from [inversion], thereby
    asking Coq to choose names for the variables and hypotheses that 
    it introduces. *)
...
(** We could try to proceed by case analysis or induction on [n].  But
    ...
    [even].  Indeed, the first case can be solved trivially. *)
...
Proof.
 (* SOLUTION: *)
  ...
  - (* -> *)
    ...
    + (* even'_0 *) apply ev_0.
    + (* even'_2 *) apply ev_SS. apply ev_0.
    + (* even'_sum *) apply (ev_sum _ _ IHn IHm).
  - (* <- *)
...
(* SOLUTION: *)
  ...
  - intros m p q H.
    ...
    + right.
      assert (H1: m + S n <= q).
      {  rewrite plus_comm. simpl. apply H.  }
      destruct (plus_le _ _ _ H1) as [H2 H3].  apply H2. 
    + simpl in H. 
     apply Sn_le_Sm__n_le_m in H.
     destruct (IHn _ _ _ H) as [H0|H0].
...
(* SOLUTION: *)
 ...
 intros n1 n2 m H. 
 split.
 - assert (H1: S n1 + n2 <= m).  { apply H. }
   destruct (plus_le _ _ _ H1) as [H2 H3]. 
   apply H2. 
 - assert (H1: n1 + S n2 <= m).
...
(* SOLUTION: *)
  ...
  - intros H. induction H.
    ...
    + (* c4 *) simpl in IHR. rewrite <- plus_n_Sm in IHR.
      injection IHR. intro Hmno. rewrite <- Hmno. reflexivity.
        ...
        + (* l = x :: l' *)
          ...
          destruct (rev l') as [| x' l''] eqn: Heqrevl'.
            ...
            * (* rev l' = x'::l'' *)
              ...
              { injection Hlen. intro H2. apply H2. }
...
(* Fri 30 Aug 2019 02:46:59 PM CEST *)
