...
(* SOLUTION: 
    ...
    _Theorem_: For all natural numbers [n] and [m], if [n =? m = true],
      then [n = m].

    _Proof_ (more pedantic, arguably less clear): We argue by 
    induction on [n]. 

      - Base case: [n = 0].  We must show, for all natural numbers [m], that
        [0 =? m = true] implies [0 = m].  We proceed by cases on [m].

          - If [m = 0], we must show that [0 =? 0 = true] implies [0 = 0],
            which holds by reflexivity.
 
          - If [m = S m'] for some [m'], we must show that
            [0 =? S m' = true] implies [0 = S m']. 
            But [0 =? S m'] evaluates to [false],
            so the antecedent of this implication is [false = true], which is 
            absurd, and hence the whole implication is true.

      - Inductive case: [n = S n']. We must show that for all natural numbers
        [m], [S n' =? m = true] implies [S n' = m].

        We may assume the induction hypothesis:
        for all natural numbers [m], [n' =? m = true],
        implies [n' = m].   

        We again proceed by cases on [m].

          - If [m = 0], we must show that [S n' =? 0 = true] implies [S n' = m]. But
            [S n' =? 0] evaluates to [false], so the antecedent of this implies
            is again absurd, and hence the whole implication is true.

          - If [m = S m'] for some [m'], we must show that 
            [S n' =? S m' = true] implies [S n' = S m'].  So let
            us assume the [S n' =? S m' = true].  This simplifes to
            [n' =? m' = true]. Hence we can apply the induction hypothesis 
            (with [m] instantiated to [m']) to obtain [n' = m'].  Hence,
            to show [S n' = S m'] it suffices to show [S n' = S n'], which
            is true by reflexivity. []

    _Proof_ (somewhat more natural style): By induction on [n].

      - Suppose [n = 0].  We must show that if [0 =? m = true] then
        [0 = m]. Now if [m] were of the form [S m'] for some [m'], then
        we would have [0 =? S m' = true], which is absurd. So [m] must indeed be 0.

      - Otherwise, we have [n = S n']. The induction hypothesis states that 
        for all m, if [n' =? m = true], then [n' = m]; and
        on the assumption [S n' =? m = true], we must show that [S n' = m].  
        In this case [m] must have the form [S m'] for some [m'], for if
        [m] were 0, our assumption would be [S n' =? 0 = true], which is absurd.
        So our assumption has the form  [S n' =? S m' = true], which simplifies
        to [n' =? m' = true]. Applying the induction hypothesis to the assumption 
        (with [m] instantiated to [m']) gives us that [n' = m'], which directly
        implies our goal [S n' = S m']. [] *)
...
(* Fri 30 Aug 2019 02:46:57 PM CEST *)
