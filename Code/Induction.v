...
Proof. 
  ...
  - (* n = S n *)
    simpl. rewrite -> IHn'. rewrite -> plus_distr. reflexivity. Qed.
...
Proof.
  ...
  - (* n = 0 *)
    simpl. reflexivity.
  - (* n = S n' *)
    rewrite -> IHn. simpl. rewrite -> negb_negb. reflexivity. Qed.
...
(* Answer: Both are used to perform case analysis on an element of an inductively defined type;
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)  
    ...
    [] *)
Theorem eqb_refl_informal : forall n : nat,
  true = eqb n n.
Proof.
...
Theorem plus_swap1 : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
...
Proof.
  intros. simpl. rewrite -> plus_n_O. reflexivity. Qed.
...
Proof.
  intros. induction n.
  - reflexivity.
  - simpl. rewrite -> bin_comm. rewrite -> IHn. reflexivity. Qed.
